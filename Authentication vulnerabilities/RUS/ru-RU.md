...
---
# Уязвимости в системе авторизации по паролю

На сайтах, использующих вход по паролю, пользователи либо регистрируются самостоятельно, либо получают учетную запись от администратора. Эта учетная запись связана с уникальным именем пользователя и секретным паролем, который пользователь вводит в форму входа для аутентификации.

В этом сценарии знание секретного пароля считается достаточным доказательством личности пользователя. Это означает, что безопасность веб-сайта скомпрометирована, если злоумышленнику удастся получить или угадать учетные данные для входа другого пользователя.


---
# Несовершенная защита от перебора паролей

Весьма вероятно, что атака методом перебора потребует множества неудачных попыток, прежде чем злоумышленник успешно взломает учетную запись. Логично, что защита от атак методом перебора заключается в том, чтобы максимально усложнить процесс, автоматизировать его и замедлить скорость попыток входа в систему. Два наиболее распространенных способа предотвращения атак методом перебора:

Блокировка учетной записи, к которой пытается получить доступ удаленный пользователь, если он совершает слишком много неудачных попыток входа в систему.
Блокировка IP-адреса удалённого пользователя, если он совершает слишком много попыток входа в систему подряд.
...
Например, иногда ваш IP-адрес может быть заблокирован, если вы слишком часто не можете войти в систему. В некоторых реализациях счетчик неудачных попыток сбрасывается, если владелец IP-адреса успешно входит в систему. Это означает, что злоумышленнику достаточно будет входить в свою учетную запись каждые несколько попыток, чтобы предотвратить достижение этого лимита.

В этом случае простое включение собственных учетных данных для входа в систему через равные промежутки во всем списке слов делает эту защиту практически бесполезной.
